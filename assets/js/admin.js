...
// ---------- estado del recortador de fotos ----------
let colaRecorte = [];           // archivos originales esperando ser encuadrados
let fotosNuevasProcesadas = []; // archivos ya recortados, listos para subir
let cropEstado = { scale: 1, scaleMin: 1, scaleMax: 3, offsetX: 0, offsetY: 0, naturalW: 0, naturalH: 0, viewportSize: 0 };
let cropArrastre = null; // { startX, startY, offsetX, offsetY } mientras se arrastra
...
function resizeImagen(file, maxAncho = 1100, calidad = 0.82) {
  return new Promise((resolve, reject) => {
    ...
    reader.onerror = () => reject(new Error(`No se pudo leer el archivo "${file.name}". Puede estar dañado o ser un formato no soportado.`));
    img.onload = () => {
    ...
    img.onerror = () => reject(new Error(`No se pudo procesar la imagen "${file.name}". Probá con otro archivo (jpg o png).`));
    reader.readAsDataURL(file);
  });
}

async function subirImagen(file, nombreArchivo, intento = 1) {
  ...
  try {
    ...
    // "Failed to fetch" = se cortó la conexión a mitad de camino, no una
    // respuesta de error de GitHub. Reintentamos una vez antes de darnos por vencidos.
    if (intento < 2) {
      await new Promise((r) => setTimeout(r, 1500));
      return subirImagen(file, nombreArchivo, intento + 1);
    }
    throw new Error(`Se cortó la conexión subiendo "${file.name}". Revisá tu wifi/datos y probá de nuevo.`);
  }
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
...
// ---------- recortador de fotos (arrastrar + zoom antes de subir) ----------

function iniciarColaRecorte(archivos) {
  colaRecorte = Array.from(archivos);
  procesarSiguienteRecorte();
}

function procesarSiguienteRecorte() {
  if (colaRecorte.length === 0) {
    document.getElementById("crop-modal").classList.remove("open");
    return;
  }
  const file = colaRecorte[0];
  const img = document.getElementById("crop-img");
  const url = URL.createObjectURL(file);

  document.getElementById("crop-titulo").textContent = `Encuadrá la foto (${fotosNuevasProcesadas.length + 1} de ${fotosNuevasProcesadas.length + colaRecorte.length})`;

  img.onload = () => {
    const viewport = document.getElementById("crop-viewport");
    const viewportSize = viewport.clientWidth;
    const scaleMin = Math.max(viewportSize / img.naturalWidth, viewportSize / img.naturalHeight);
    cropEstado = {
      scale: scaleMin,
      scaleMin,
      scaleMax: scaleMin * 3,
      offsetX: (viewportSize - img.naturalWidth * scaleMin) / 2,
      offsetY: (viewportSize - img.naturalHeight * scaleMin) / 2,
...
function aplicarTransformCrop() {
  const img = document.getElementById("crop-img");
  const { scale, offsetX, offsetY, naturalW, naturalH } = cropEstado;
...
function clampOffsetsCrop() {
  const { scale, naturalW, naturalH, viewportSize } = cropEstado;
  const anchoEscalado = naturalW * scale;
  const altoEscalado = naturalH * scale;
  cropEstado.offsetX = Math.min(0, Math.max(viewportSize - anchoEscalado, cropEstado.offsetX));
  cropEstado.offsetY = Math.min(0, Math.max(viewportSize - altoEscalado, cropEstado.offsetY));
}

function onCropZoomInput(e) {
  const t = Number(e.target.value) / 100; // 0..1
  const nuevaScale = cropEstado.scaleMin + t * (cropEstado.scaleMax - cropEstado.scaleMin);
  const vc = cropEstado.viewportSize / 2;
  // Mantiene el punto de la imagen que está en el centro del visor fijo al hacer zoom.
  const ix = (vc - cropEstado.offsetX) / cropEstado.scale;
  const iy = (vc - cropEstado.offsetY) / cropEstado.scale;
  cropEstado.scale = nuevaScale;
  cropEstado.offsetX = vc - ix * nuevaScale;
  cropEstado.offsetY = vc - iy * nuevaScale;
  clampOffsetsCrop();
  aplicarTransformCrop();
}
...
function onCropPointerDown(e) {
  ...
  cropArrastre = { startX: e.clientX, startY: e.clientY, offsetX: cropEstado.offsetX, offsetY: cropEstado.offsetY };
}

function onCropPointerMove(e) {
  if (!cropArrastre) return;
  cropEstado.offsetX = cropArrastre.offsetX + (e.clientX - cropArrastre.startX);
  cropEstado.offsetY = cropArrastre.offsetY + (e.clientY - cropArrastre.startY);
  clampOffsetsCrop();
  aplicarTransformCrop();
}
...
function confirmarCrop() {
  const img = document.getElementById("crop-img");
  const { scale, offsetX, offsetY, viewportSize } = cropEstado;
  ...
  const fileOriginal = colaRecorte.shift();
  canvas.toBlob(
    (blob) => {
      const nombreBase = fileOriginal.name.replace(/\.[^.]+$/, "") || "foto";
      const fileRecortado = new File([blob], `${nombreBase}.jpg`, { type: "image/jpeg" });
      fotosNuevasProcesadas.push(fileRecortado);
      renderFotosNuevasPreview();
...
function omitirCrop() {
  const fileOriginal = colaRecorte.shift();
  URL.revokeObjectURL(document.getElementById("crop-img").src);
  procesarSiguienteRecorte();
}

function renderFotosNuevasPreview() {
  const cont = document.getElementById("f-imagenes-nuevas");
  cont.innerHTML = fotosNuevasProcesadas
    .map(
      (file, i) => `
      <div class="foto-actual-item">
        <img src="${URL.createObjectURL(file)}" alt="">
        <button type="button" class="foto-actual-quitar" onclick="quitarFotoNueva(${i})" title="Quitar esta foto">×</button>
      </div>`
    )
    .join("");
}
...
function quitarFotoNueva(indice) {
  fotosNuevasProcesadas.splice(indice, 1);
  renderFotosNuevasPreview();
}

// ---------- formulario alta / edición ----------

let imagenesActuales = []; // fotos ya guardadas de la prenda que se está editando

function renderFotosActuales() {
  const cont = document.getElementById("f-imagenes-actuales");
  cont.innerHTML = imagenesActuales
    .map(
      (path, i) => `
      <div class="foto-actual-item">
        <img src="${urlImagenAdmin(path)}" alt="">
        <button type="button" class="foto-actual-quitar" onclick="quitarFotoActual(${i})" title="Quitar esta foto">×</button>
      </div>`
    )
    .join("");
}

function quitarFotoActual(indice) {
  if (!confirm("¿Quitar esta foto de la prenda?")) return;
...
async function guardarForm(e) {
  ...
  try {
    ...
    const archivos = fotosNuevasProcesadas;

    if (!nombre || !precio) throw new Error("Completá al menos nombre y precio.");

    let id = editandoId;
    let imagenes = editandoId ? [...imagenesActuales] : [];
    ...
    if (archivos.length > 0) {
      ...
      for (let i = 0; i < archivos.length; i++) {
        btn.textContent = `Subiendo foto ${i + 1} de ${archivos.length}…`;
        const nombreArchivo = `${id}-${Date.now()}-${i}.jpg`;
    ...
    alert("Error: " + (err && err.message ? err.message : "algo salió mal, revisá tu conexión y probá de nuevo."));
    console.error(err);
  } finally {
    btn.disabled = false;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("purgar-btn").addEventListener("click", async (e) => {
    const btn = e.target;
    btn.disabled = true;
    ...
    setTimeout(() => {
      ...
      alert("Listo. Esperá unos segundos y refrescá el catálogo público.");
    }, 1200);
  });
  ...
  document.getElementById("f-imagenes").addEventListener("change", (e) => {
    if (e.target.files.length > 0) iniciarColaRecorte(e.target.files);
    e.target.value = ""; // permite volver a elegir el mismo archivo si hace falta
  });
  const cropViewport = document.getElementById("crop-viewport");
  cropViewport.addEventListener("pointerdown", onCropPointerDown);
  cropViewport.addEventListener("pointermove", onCropPointerMove);
  cropViewport.addEventListener("pointerup", onCropPointerUp);
  cropViewport.addEventListener("pointercancel", onCropPointerUp);
  document.getElementById("crop-zoom").addEventListener("input", onCropZoomInput);
  document.getElementById("crop-confirmar").addEventListener("click", confirmarCrop);
  document.getElementById("crop-omitir").addEventListener("click", omitirCrop);
